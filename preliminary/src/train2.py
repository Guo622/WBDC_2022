...
def train(net: nn.Module, trainloader,testloader, loss_fn, optimizer,scheduler, epochs,writer):
    ...
    for e in range(epochs):
        ...
        for i, input_data in enumerate(tqdm_loader):
            ...
            writer.add_scalar('train/loss_two', loss.item(), e*len(trainloader)+i)
            writer.add_scalar('train/acc_two',acc, e*len(trainloader)+i)
            tqdm_loader.set_description(
                f"epoch:{e+1} batch:{i+1} loss:{loss.item():.6f} acc:{acc:.6f}")

            if e >= swa_start and (i+1) % swa_batch==0:
                ema.apply_shadow()
                torch.save(net.state_dict(), f"check/train2/model_train_{e+1}_{i+1}.pth")
                ema.restore()
...
if __name__ == '__main__':
    ...
    config.epochs=4
    net = Model2(config).to(config.device)
    net.load_state_dict(torch.load('./check/pretrain/pretrain2.pth'),strict=False)
    trainloader= create_dataloaders(config,'train2')
    loss_fn = nn.CrossEntropyLoss().to(config.device)

    optimizer,scheduler=build_optimizer(config,net,'train2')
