...
from model import MultiModal,VisualBackbone
from config import parse_args
from data_helper import create_dataloaders,create_dataloaders_pretrain,SingleModalDataset
from util import setup_device, setup_seed, setup_logging, build_optimizer, evaluate,build_optimizer_visual_pertrain
...
def pretrain(args):
    # 1. load data
    dataset = SingleModalDataset(args, args.pretrain_annotation, args.pretrain_zip_frames,'frame')
    train_dataloader= create_dataloaders_pretrain(args,dataset)


    # 2. build model and optimizers
    model = VisualBackbone(args,'pretrain')
    optimizer, scheduler =build_optimizer_visual_pertrain(args, model)
    if args.device == 'cuda':
    ...
    for epoch in range(args.max_epochs):
        ...
        torch.save({'epoch': epoch, 'model_state_dict': state_dict},
                       f'{args.savedmodel_path}/pretrain_visual.bin')
